...
class environment:
    def __init__(self,
        ...
        self.number_of_sbs=np.shape(bs_list)[0]
    ...
    def compute_distance(self,x,y):
        ...
        return distance
    def user_coming1(self, user_coming,beta=-5):
        temp=user_coming.iloc[:,[0,1,2]]
        selected_bs = self.user_association(np.array(temp.ix[:, ['x', 'y']]), beta)
        temp['selected_BS'] = selected_bs
        self.user_list = self.user_list.append(temp)
        self.get_user_number_per_bs()
    ...
    def reset(self,bs_list,system_time=0,mbs_load=0):
        self.mbs_load = mbs_load
        self.bs_list = bs_list
        self.user_list = pd.DataFrame(columns=['x', 'y', 'access time', 'selected_BS'])
    ...
    def step(self, action,user_coming, beta=-8):

        self.change_bs_on(action)

        self.user_coming1(user_coming,beta)
    ...
    def choose_action(self):
        action=np.array(self.bs_list.iloc[:,2])
        for i in range(self.number_of_sbs):
            temp=self.bs_list.iloc[i,3]
            if temp >800.0:
                action[i]=1
            elif temp<300.0:
                action[i]=0
